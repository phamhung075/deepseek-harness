...
| `watchPollIntervalMs` | `500` | Safety-net poll cadence for `ctx.sessionAppends` and `ctx.sessionStreams`; a reported append is observed immediately |
| `streamPendingRecords` | `512` | Maximum live-frame records one publishing Session buffers while a write is in flight; frames past the bound are dropped |
...
`open(id, 'read'|'write')` selects the highest canonical generation. Current input follows the ordinary fast path. For historical input, a read open decodes and migrates the source once, validates the current logical result, and returns it without publishing a successor. A write open reuses that revision-keyed preparation when available, or performs the same preparation, then encodes a same-directory temporary file in bounded chunks, verifies it in a Worker Thread, rechecks the source revision, and publishes the current successor without overwrite before returning. The source remains byte-identical. Source drift after preparation rejects that write open without replacing the logical history already returned to readers; a later write open prepares the new revision. The backend marks decoded event graphs `shared-frozen` when it freezes them before memoization; handle reads and slices preserve that state, including empty slices. Only an unmaterialized pending log reports `detached`. `stat(id)` and `list()` select and translate only the highest generation header without reading event rows or starting migration; snapshots carry `sizeBytes`, `lastModifiedAt`, and a best-effort stat-derived revision for the selected file. With `compression: 'none'`, the log is newline-delimited text an external reader can consume directly; the compressed default must be read through the backend.

### Observing foreign appends

The backend also registers `ctx.sessionAppends` (from `src/appends.ts`), the optional companion capability the Session Controller reads to report and stream a session another process is running. `watch(id)` anchors a subscription at the current generation's durable end and yields the logical events each later append makes durable, decoding only the appended bytes: complete Zstandard frames past a held byte offset, and complete lines for `compression: 'none'`, with an unfinished trailing record withheld until the bytes completing it land (`AppendTailReader` in `src/append-tail.ts` decodes through the anchored `AppendedRowDecoder` in `src/format.ts`). A `fs.watch` on the session directory reports appends promptly and `watchPollIntervalMs` (default `500`) bounds the wait when the watcher is silent or unavailable. A repair that truncates the artifact, a replaced file, or an append that does not begin a decodable record restarts the reader from byte 0 instead of guessing, which may replay events a consumer already saw, while an append that re-emits a seq this stream already reported is dropped, so a writer that resumed an interrupted turn can neither end the subscription nor move its position; only an append ahead of the expected seq fails the stream. The subscription, its watcher, its timer, and its read loop are released by `close()` and by backend teardown, which waits for the read loop to settle.

### Publishing and reading live frames

`ctx.sessionStreams` (from `src/streams.ts`) is the second optional companion: it carries the process-local `agent/assistant-stream` frames a publisher (the ACP profile) writes beside the artifact, so a Host that did not run the Agent can stream its text. The channel is `<generation stem>.stream.jsonl` in the Session directory — for example `session.v3.stream.jsonl` — a plaintext JSONL file of `{"seq": <next durable seq>, "frame": <frame>}` records. No generation parser accepts the name, so it can never be read as the durable log, and it is removed with the Session directory. `publish(id)` returns an owned sink whose path is resolved on the first frame, so a Session that has not materialized yet is not an error; writes are ordered behind one in-flight flush and bounded by `streamPendingRecords`, and a frame past the bound is dropped. The first write repairs an incomplete trailing record left by a writer killed mid-frame, so the next record is never concatenated onto a partial line. `watch(id)` tails complete lines through `StreamTailReader`, withholds an incomplete trailing line, restarts on a shorter or replaced file, and shares its subscription machinery with the append observer (`src/tail-subscription.ts`).
...
| [`src/append-tail.ts`](src/append-tail.ts) | The incremental append reader: framed/line decoding past a held byte offset and the expected-seq cursor |
| [`src/appends.ts`](src/appends.ts) | The `ctx.sessionAppends` provider: watcher, poll safety net, and owned subscriptions |
| [`src/streams.ts`](src/streams.ts) | The `ctx.sessionStreams` provider: publishing sinks and live-frame subscriptions |
| [`src/stream-channel.ts`](src/stream-channel.ts) | The side-channel file: naming, torn-tail repair, the bounded writer, and the line tail reader |
| [`src/tail-subscription.ts`](src/tail-subscription.ts) | The shared file-tail subscription both observers drive |
| [`src/tail-reader.ts`](src/tail-reader.ts) | The shared append-only file skeleton: anchoring, appended-byte reads, and restart on truncation or replacement |
...
- **An unaligned append restarts the observer from byte 0** — a repair, a replaced artifact, or a subscription that opened inside a rewritten frame replays the log rather than guessing a position, so a consumer must tolerate events at or below its own cursor.
- **The live-frame channel is presentation data, never replay** — a frame dropped at `streamPendingRecords`, or lost with a writer killed before its line completed, has no repair path; the client re-synchronizes from the durable settlement the log embeds.
